...
- Attachments use **`MpUpload`** (`@change` gives a native input event → `event.target.files`), _not_ `MpDropzone` — `MpUpload` renders the "Choose file" button + inline placeholder that the Jurnal forms use; `MpDropzone` is the drop-area/avatar/logo variant.
