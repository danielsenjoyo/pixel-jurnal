...
Used by `/pixel-review`. Never guess from memory — always read this file first.

This file is meant to travel with `pixel-review.js` and `report-template.html` as one portable asset
set. A repo outside Mekari/Qontak using `/pixel-review` should replace the CHOICE Principles source
below with its own design principles (or its own live doc URL) — everything else (NNG, scoring
reference, severity scale) is framework-agnostic and can be reused as-is.

## Mekari CHOICE Principles

Source: https://docs.superhuman.com/d/Mekari-UI-UX_dP1GCpeftMT/Objective-and-Principles_suTDCYRt
(Mekari internal doc — verified against the live doc 2026-08-18). Used as-is, no live fetch at review
time; update this list by hand if the source doc changes materially.

- **Clear:** User tahu apa yang harus dilakukan dalam 5 detik pertama
- **Holistic:** Konsisten dengan produk Mekari lainnya (Talenta, Jurnal, Klikpajak, dsb)
